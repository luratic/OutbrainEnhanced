___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "displayName": "Outbrain Enhanced",
  "description": "Outbrain Pixel allows you to track conversions and create Custom Audiences on your web site.\nThis version allow advanced options.",
  "categories": [
    "ATTRIBUTION",
    "REMARKETING"
  ],
  "securityGroups": [],
  "id": "cvt_temp_public_id",
  "type": "TAG",
  "version": 1,
  "brand": {
    "thumbnail": "data:image/png;base64,",
    "displayName": "Custom Template",
    "id": "brand_custom_template"
  },
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "notSetText": "You must set the Marketer Id field.",
    "help": "You can find your Marketer Id on the Amplify Dashboard. Navigate to the Conversions page and click on the \"View Outbrain Pixel\" button. Then you can copy your Marketer Id from the code snippet (It is a string of numbers and letters named \"OB_ADV_ID\").",
    "valueValidators": [
      {
        "args": [
          "([0-9a-zA-Z]{10,50},)*[0-9a-zA-Z]{10,50}$"
        ],
        "type": "REGEX"
      },
      {
        "type": "NON_EMPTY"
      }
    ],
    "displayName": "Marketer Id (OB_ADV_ID)",
    "simpleValueType": true,
    "name": "MarketerId",
    "type": "TEXT"
  },
  {
    "type": "RADIO",
    "name": "type",
    "displayName": "Select an event:",
    "radioItems": [
      {
        "value": "PAGE_VIEW",
        "displayValue": "Pageview",
        "help": "Track a PAGE_VIEW event."
      },
      {
        "value": "Purchase",
        "displayValue": "Purchase",
        "help": "Track a purchase event.",
        "subParams": []
      },
      {
        "value": "customEvent",
        "displayValue": "Custom Event",
        "help": "Custom Event Name",
        "subParams": [
          {
            "type": "TEXT",
            "name": "name",
            "displayName": "Custom Event Name:",
            "simpleValueType": true,
            "help": "Set a custom event name",
            "valueHint": "Register"
          }
        ]
      }
    ],
    "simpleValueType": true
  },
  {
    "type": "GROUP",
    "name": "info",
    "displayName": "Additional parameters",
    "subParams": [
      {
        "type": "SIMPLE_TABLE",
        "name": "parameters",
        "displayName": "",
        "simpleTableColumns": [
          {
            "defaultValue": "",
            "displayName": "Name parameter",
            "name": "name",
            "type": "TEXT",
            "isUnique": true,
            "valueHint": "currency"
          },
          {
            "defaultValue": "",
            "displayName": "Value parameter",
            "name": "value",
            "type": "TEXT",
            "valueHint": "EUR"
          }
        ]
      }
    ],
    "enablingConditions": [
      {
        "paramName": "type",
        "paramValue": "Purchase",
        "type": "EQUALS"
      },
      {
        "paramName": "type",
        "paramValue": "customEvent",
        "type": "EQUALS"
      }
    ],
    "groupStyle": "ZIPPY_OPEN"
  },
  {
    "type": "LABEL",
    "name": "customInfo",
    "displayName": "\u003ca href\u003d\"https://github.com/precariostecnicos/OutbrainEnhanced\"\u003eInfo \u003c/a\u003e"
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const injectScript = require('injectScript');
const setInWindow = require('setInWindow');
const log = require('logToConsole');
const makeTableMap = require('makeTableMap');
const copyFromWindow = require('copyFromWindow');
const callInWindow = require('callInWindow');
log("data", data);
const getEventName  = () => {
  const type = data.type;
  if(type === 'customEvent') {
    return data.name;
  }
  return type;
};
const getParameters = () => {
  var parameterTable = data.parameters;
  if(parameterTable) {
	return makeTableMap(parameterTable, 'name', 'value');
  }
  return false; 
};
const initApi = () => {
  const api = function () {
    const apiDispatch = copyFromWindow('obApi.dispatch');
    if(apiDispatch) {
      callInWindow('obApi.dispatch.apply', api, arguments); 
    }
    else {
      callInWindow('obApi.queue.push', arguments);
    }	    
  }; 
  api.version = '1.1';
  api.loaded = true;
  api.marketerId = id.split(',');
  api.queue = [];
  return api; 
};
const getApi = () => {
  let obApi  = copyFromWindow('obApi');
  if(obApi) {
    return obApi;
  }
  const api = initApi();
  setInWindow('obApi', api);
  return copyFromWindow('obApi');
};
const sendEvent = () => {
  if(parameters) {
     obApi('track', eventName, parameters);     
  }
  else {
    obApi('track', eventName);      
  }  
};
const id = data.MarketerId; 
const eventName = getEventName(); 
const parameters = getParameters();
log("parameters", parameters);
log('eventName', eventName);
const url = 'https://amplify.outbrain.com/cp/obtp.js';

const obApi = getApi();
sendEvent();
injectScript(url, data.gtmOnSuccess, data.gtmOnFailure, 'outbrain');


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "obApi"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "obApi.queue"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "obApi.dispatch"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "obApi.dispatch.apply"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "obApi.queue.push"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "inject_script",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://amplify.outbrain.com/*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "isRequired": true
  }
]


___TESTS___

scenarios:
- name: gtmOnSuccess is invoked on injectScript success
  code: |-
    // test that gtmOnSuccess() is called when injectScript succeeds
    mock('injectScript', (url, onSuccess, onFailure) => {
      onSuccess();
    });

    // Call runCode to run the template's code.
    runCode();

    // Verify that the tag finished successfully.
    assertApi('gtmOnSuccess').wasCalled();
- name: gtmOnFailure is invoked when injectScript fails
  code: |-
    // test gtmOnFailure() is called when injectScript fails
    mock('injectScript', (url, onSuccess, onFailure) => {
      onFailure();
    });

    // Call runCode to run the template's code.
    runCode();

    // Verify onFailure was called
    assertApi('gtmOnFailure').wasCalled();


___NOTES___

Developed by: Alfonso, Txema and Brais


